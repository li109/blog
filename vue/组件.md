...
```
&emsp;&emsp;该函数对组件的处理比较有意思，首先是关于组件名称的问题：**在模板中使用的组件名称，在组件注册时可以有三种形式**。注册时可以跟跟使用时保持一致，也可以使用驼峰命名或者首字母大写的驼峰命名。<br/>
...
&emsp;&emsp;接着调用 *extractPropsFromVNodeData* 函数来从当前实例中提取局部组件 *props* 的值，调用 *installComponentHooks* 来在 *data* 属性上安装组件的钩子函数，具体的钩子函数以及调用会在下一节讲解生成真实DOM时阐述。<br/>
